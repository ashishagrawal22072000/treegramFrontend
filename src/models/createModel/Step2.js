  ...
  const handleSubmit = (username) => {
    ...
    if (tags.length) {
      const tag = tags.find((ele) => {
        return ele.username == username && ele.media_url == selectedImage;
      });
      if (tag) {
        Notify("error", `You have already selected ${username}`);
      } else {
        const data = {
          username,
          media_url: selectedImage,
          coordinate_x: coordinate.coordinate_x,
              ...
              <div>
                ...
                <div className="">
                  {data.map((ele) => {
                    return (
                      <>
                        <h4 onClick={() => handleSubmit(ele.username)}>
                          {ele.username}
                        </h4>
                      </>
                    );
